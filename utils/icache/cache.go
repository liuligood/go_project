...
func GetKeyStatus(key string) KeyInfo {
	...
	if err != nil {
		izap.Log.Error("Error getting key:", zap.Error(err))
	}
...
func GetCache(ctx context.Context, key string, redisClient redis.UniversalClient) (string, error) {
...
func GetCacheOrElse(ctx context.Context, key string, ttl time.Duration, fetcher Fetcher, redisClient redis.UniversalClient) (string, error) {
	value, err := GetLocal(key)
	if err != nil {
		if !errors.Is(err, freecache.ErrNotFound) {
			return "", err
	...
	if err != nil {
		if errors.Is(err, redis.Nil) {
			...
			SetCache(ctx, key, value, ttl, redisClient)
			return value, nil
		} else {
			return "", err
		}
...
func SetCache(ctx context.Context, key, value string, ttl time.Duration, redisClient redis.UniversalClient) error {
	if len(key) == 0 {
		return errors.New("key is empty")
	}
	...
	result, err := redisClient.Set(ctx, key, value, ttl).Result()

	izap.Log.Info("redis2 set", zap.String("result", result))
	if err != nil {
		return err
	}
...
func DelCache(key string, redisClient redis.UniversalClient) error {
	if len(key) == 0 {
		return errors.New("key is empty")
	}
	...
	izap.Log.Info("redis2 del num", zap.Int64("result", result))
