...
func NewGormZapLogger(zapLogger *zap.Logger, logger gormlogger.LogLevel) gormlogger.Interface {
	return &Logger{
		ZapLogger:                 zapLogger,              // zap日志
		LogLevel:                  logger,                 // 日志等级
		SlowThreshold:             200 * time.Millisecond, // 慢 SQL 阈值
		Colorful:                  true,                   // 彩色打印
		IgnoreRecordNotFoundError: true,                   // 是否忽略RecordNotFoundError
		ParameterizedQueries:      true,                   // 是否打印参数
	}
}
