...
const TableNameStoreSeckill = "store_seckill"

// StoreSeckill 商品秒杀产品表
type StoreSeckill struct {
	ID           int64                 `gorm:"column:id;type:int(10) unsigned;primaryKey;autoIncrement:true;comment:商品秒杀产品表id" json:"id"`                 // 商品秒杀产品表id
	...
	Status       int64                 `gorm:"column:status;type:tinyint(3) unsigned;not null;comment:秒杀状态 0=关闭 1=开启" json:"status"`                      // 秒杀状态 0=关闭 1=开启
	IsPostage    int64                 `gorm:"column:is_postage;type:tinyint(3) unsigned;not null;comment:是否包邮" json:"is_postage"`                        // 是否包邮
	Num          int64                 `gorm:"column:num;type:int(10) unsigned;not null;comment:当天参与活动次数" json:"num"`                                     // 当天参与活动次数
	IsShow       int64                 `gorm:"column:is_show;type:tinyint(3) unsigned;not null;default:1;comment:显示" json:"is_show"`                      // 显示
	TimeID       int64                 `gorm:"column:time_id;type:int(11);comment:时间段ID" json:"time_id"`                                                  // 时间段ID
	...
	Quota        int64                 `gorm:"column:quota;type:int(11);not null;comment:限购总数,随减" json:"quota"`                                           // 限购总数,随减
	QuotaShow    int64                 `gorm:"column:quota_show;type:int(11);not null;comment:限购总数显示.不变" json:"quota_show"`                               // 限购总数显示.不变
