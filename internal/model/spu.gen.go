...
type Spu struct {
	...
	ServerID    int64                 `gorm:"column:server_id;type:int unsigned;not null;comment:商家id" json:"server_id"`                        // 商家id
	CategoryID  int64                 `gorm:"column:category_id;type:int unsigned;not null;comment:分类id" json:"category_id"`                    // 分类id
	BrandID     int64                 `gorm:"column:brand_id;type:int unsigned;not null;comment:品牌id" json:"brand_id"`                          // 品牌id
	ModelID     int64                 `gorm:"column:model_id;type:int unsigned;not null;comment:型号id" json:"model_id"`                          // 型号id
	Keyword     string                `gorm:"column:keyword;type:varchar(255);not null;comment:关键词" json:"keyword"`                             // 关键词
	Status      int64                 `gorm:"column:status;type:tinyint unsigned;not null;comment:状态(1:上架中,2:待审核,3:商家下架,4:系统下架)" json:"status"` // 状态(1:上架中,2:待审核,3:商家下架,4:系统下架)
	Sort        int64                 `gorm:"column:sort;type:int unsigned;not null;comment:排序" json:"sort"`                                    // 排序
	Sign        int64                 `gorm:"column:sign;type:int unsigned;not null;comment:spu标志（位运算）" json:"sign"`                            // spu标志（位运算）
	Scene       int64                 `gorm:"column:scene;type:int unsigned;not null;comment:场景（位运算）" json:"scene"`                             // 场景（位运算）
	...
	Version     int64                 `gorm:"column:version;type:int unsigned;not null;comment:版本" json:"version"`                              // 版本
