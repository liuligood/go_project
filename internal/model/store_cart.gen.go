...
type StoreCart struct {
	ID                int64                 `gorm:"column:id;type:bigint(20) unsigned;primaryKey;autoIncrement:true;comment:购物车表ID" json:"id"`    // 购物车表ID
	...
	CartNum           int64                 `gorm:"column:cart_num;type:smallint(5) unsigned;not null;comment:商品数量" json:"cart_num"`              // 商品数量
	IsNew             int64                 `gorm:"column:is_new;type:tinyint(1);not null;comment:是否为立即购买" json:"is_new"`                         // 是否为立即购买
	CombinationID     int64                 `gorm:"column:combination_id;type:int(10) unsigned;comment:拼团id" json:"combination_id"`               // 拼团id
