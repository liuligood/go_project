...
type Sku struct {
	...
	SpuID     int64                 `gorm:"column:spu_id;type:int unsigned;not null" json:"spu_id"`
	...
	Status    int64                 `gorm:"column:status;type:tinyint unsigned;not null;comment:状态(1:关闭,2:正常)" json:"status"`         // 状态(1:关闭,2:正常)
	Sign      int64                 `gorm:"column:sign;type:int unsigned;not null;comment:标志(位运算)" json:"sign"`                       // 标志(位运算)
