...
type SystemNotification struct {
	...
	IsWechat    int64                 `gorm:"column:is_wechat;type:tinyint(4);not null;comment:公众号模板消息（0：不存在，1：开启，2：关闭）" json:"is_wechat"`   // 公众号模板消息（0：不存在，1：开启，2：关闭）
	WechatID    int64                 `gorm:"column:wechat_id;type:int(11);not null;comment:模板消息id" json:"wechat_id"`                        // 模板消息id
	IsRoutine   int64                 `gorm:"column:is_routine;type:tinyint(4);not null;comment:小程序订阅消息（0：不存在，1：开启，2：关闭）" json:"is_routine"` // 小程序订阅消息（0：不存在，1：开启，2：关闭）
	RoutineID   int64                 `gorm:"column:routine_id;type:int(11);not null;comment:订阅消息id" json:"routine_id"`                      // 订阅消息id
	IsSms       int64                 `gorm:"column:is_sms;type:tinyint(4);not null;comment:发送短信（0：不存在，1：开启，2：关闭）" json:"is_sms"`            // 发送短信（0：不存在，1：开启，2：关闭）
	SmsID       int64                 `gorm:"column:sms_id;type:int(11);not null;comment:短信id" json:"sms_id"`                                // 短信id
	SendType    int64                 `gorm:"column:send_type;type:tinyint(4);not null;default:1;comment:发送类型（1：用户，2：管理员）" json:"send_type"` // 发送类型（1：用户，2：管理员）
